...
deterministic, observe-only mirror of operator↔XO traffic into a durable,
append-structured "who-knows-what" context ledger the CoS reads and integrates. The
...
change — it only records traffic the relay and `notify` already handle.
