...
- [x] 1.1 Define the `guildAPI` seam interface (`selfMember`, `guild`, `channels`,
...
- [x] 1.2 Implement `effectivePermissions(g, botID, botRoles) int64` — MIRROR discordgo's
...
- [x] 1.3 Implement `hasManageChannels(perms) bool` and the name `key(name, ctype)` — text →
...
- [x] 1.4 Implement `findExisting(chans, name, ctype, parentID) (Channel, bool)`. TEST FIRST:
      match on normalized name, type mismatch, parent mismatch, same-name-different-parent.
- [x] 1.5 Implement `Provisioner.Preflight(guildID)` over the seam (selfMember + guild →
      effectivePermissions). TEST FIRST with a fake `guildAPI`: pass (owner/admin/grant),
      fail (no Manage Channels) → clear error.
- [x] 1.6 Implement `Provisioner.Create(guildID, spec) (Channel, created bool, err)` — list →
...
- [x] 1.7 Implement `Provisioner.List` and `Provisioner.Delete` over the seam (Delete maps a
      404 to a clear "no such channel" error). TEST FIRST.
- [x] 1.8 Implement `bindingSnippet(channelID, xo, members, role)` → indented `roster.Channel`
      JSON. TEST FIRST: valid JSON, correct fields, empty members/role omitted.
- [x] 1.9 Implement the `discordgoAPI` adapter (wraps `*discordgo.Session`) and
...
- [x] 1.10 Secret-discipline test: the returned error chain for a simulated REST failure
...
- [x] 2.1 `parseChannelCreateArgs` / `parseChannelDeleteArgs` (pure). TEST FIRST: name
...
- [x] 2.2 `cmdChannel(args)` dispatch (create|list|delete; unknown sub → clear error).
- [x] 2.3 `cmdChannelCreate`: precedence-ordered checks (token → guild_id), load roster
...
- [x] 2.4 `cmdChannelList`: load roster+secrets, `List`, print table or `--json`.
- [x] 2.5 `cmdChannelDelete`: require `--yes` + snowflake, load roster+secrets, `Delete(id)`,
      print `deleted <id>` (clear error on 404).

## 3. Wiring + docs

- [x] 3.1 Add `case "channel": return cmdChannel(args[1:])` to `cmd/flotilla/main.go` `run()`.
- [x] 3.2 Add the `channel` usage block (create/list/delete + flags) to `usage()`.
- [x] 3.3 Update `README.md` (and any command reference) with the `flotilla channel` surface
      and the provision→wire-binding flow.
- [x] 3.4 `gofmt`, `go vet ./...`, `go build ./...`, `go test ./...` all green.
