# federation Specification (delta: new capability)

> Adds the `federation` capability: per-XO + fleet-command Discord channels, the
> recursive meta-XO → project-XOs → desks hierarchy, uniform addressing, the config
> surface, and the cross-tier delivery transport. The inbound relay mechanics live
> in the `watch` capability (see the companion delta); this capability owns the
> topology, the model, and the config.
