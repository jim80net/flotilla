...
> The IMPLEMENTATION plan for PR2 (#188 / #106), now EXECUTED in this change.
> Each is bite-sized TDD: a failing test first, then the minimal code. Every
> preserved dash behavior is byte-pinned (a green test on a half-migrated seam is
> the scaffold trap). Sections 1–6 are implemented; section 7 is the close-out gate.
...
- [x] 1.1 Pin the CURRENT dash notify behavior as the baseline. Confirm the
...
- [x] 1.2 Write a FAILING test: the control library's `post` seam is satisfied by
...
- [x] 1.3 Re-point `internal/dash/control/library.go`: replace the `post`
...
- [x] 1.4 Add `internal/dash/control/no_discord_import_test.go` mirroring
      `internal/watch/no_discord_import_test.go`: assert the control library no
      longer imports `internal/discord`.
- [x] 1.5 Re-run 1.1's suites — they MUST pass UNCHANGED (the re-point is
...
- [x] 2.1 Write a FAILING test: `transport.Get("web")` resolves a constructed web
...
- [x] 2.2 Add `internal/transport/web.go`: a `webTransport` with an `init()`
      calling `RegisterFactory("web", newWebTransport)` (mirroring
      `discord.go:17-19`), and a `Name()` returning `"web"`.
- [x] 2.3 Define `webDestination` (the opaque `Destination`, `transport.go:89`)
...
- [x] 2.4 Write a test PINNING the direction: a `webDestination` carries no
...
- [x] 3.1 Write a FAILING test: `webTransport.ResolveDestination(originChannel,
...
- [x] 3.2 Implement `webTransport.ResolveDestination` by extracting the dash's
...
- [x] 3.3 Write a test asserting the web transport does NOT implement `CatchUp`
...
- [x] 3.4 Write a test PINNING `webTransport.Subscribe` as a NO-OP: it opens no
...
- [x] 4.1 Pin the CURRENT dash route delivery as the baseline. Confirm the
...
- [x] 4.2 Confirm the web inbound delivery reuses the SAME confirmed-delivery
...
- [x] 5.1 Byte-pin the dash's security tests UNCHANGED — they are the web
      ...
      MUST pass UNCHANGED.
- [x] 5.2 Add a graft-level test: a web coordination instruction arrives via
...
- [x] 6.1 Write a FAILING test for the dash command wiring: `cmd/flotilla/dash.go`
      ...
      post medium.)
- [x] 6.2 Implement the dash wiring: construct the discord-backed transport +
...
- [x] 6.3 Add a coexistence test: a `web`-constructed transport and a
...
- [x] 7.1 `openspec validate web-transport --strict` passes.
- [x] 7.2 `bash scripts/check-private-boundary.sh` passes (no deployment
      identifiers introduced; generic roles only).
- [x] 7.3 Full `go test ./...` green — every byte-pinned dash suite UNCHANGED,
