...
- [ ] 1.1 `encodeProjectDir(cwd) string` — every `/` and `.` → `-`. TEST against the probe-verified
      pairs (e.g. `/home/jim/workspace/github.com/jim80net/flotilla-dash` →
      `-home-jim-workspace-github-com-jim80net-flotilla-dash`).
- [ ] 1.2 `LatestSession(cwd) (path string, ok bool)` — glob `~/.claude/projects/<enc>/*.jsonl`,
...
- [ ] 1.3 `lastTurnText(jsonlPath) (string, ok bool)` — reverse-walk to the last `type=="assistant"`
...
- [ ] 1.4 `stripAndClassify(text) (clean string, substantive bool)` — strip
...
- [ ] 1.5 `LatestTurnText(pane) (text string, ok bool, err error)` — compose 1.1–1.4 via
      `deliver.PaneCWD(pane)`. ok=false when no session / no substantive turn-final.

## 2. `internal/discord` — bounded chunker (TDD)

- [ ] 2.1 `ChunkContent(text string, limit int) []string` — split on paragraph (`\n\n`) boundaries;
...
- [ ] 3.1 Implement `surface.ResultReader` on the claude-code driver via
...
- [ ] 4.1 Add `MirrorOnFinish func(agent string)` to `DetectorConfig` (default nil ⇒ inert). In
...
- [ ] 5.1 Wire `MirrorOnFinish`: `secrets.Webhook(agent)` (skip if none) → `claudestore.LatestTurnText`
...
- [ ] 6.1 `go build ./... && go test ./... -race` green; `go vet` clean.
- [ ] 6.2 `openspec validate desk-mirror-tier1 --strict`.
