...
- [x] 1.1 TEST: `parsePanelFocused` — the golden verified-live backend capture (empty composer
  above; panel docked at bottom; cursor on the last agent row `❯ ◯ desk-b`) → (true, true).
- [x] 1.2 TEST: LONG panel (8 agent rows — the data case) with the cursor on a MIDDLE row → still
...
- [x] 1.4 TEST: scrollback echo, two flavors → (false, true): (a) a lone `❯ ◯ …` line above a live
  ...
  NO live panel (the proven frontend false positive — the case that breaks header-anchoring). The
