...
Captured from the un-mutated `backend` (%31) pane, 2026-06-22 (the operator-facing desk the
...
2. When the panel is FOCUSED, its cursor row is the **bottom-most** chrome (panel docks below the
   ...
   (if shown) carry NO `❯`.
3. `parseComposerPending`'s current bottom-up scan lands on `❯ ◯ desk-b` → reports
...
On the stuck `data` pane, via `tmux send-keys`, NONE of these cleared the panel:
...
- **Header-anchoring false-positives on a full-panel echo with NO live panel.** STORM S2 assumed
  ...
  (the verified `frontend` case), the bottom-most header IS the echoed one, and scanning below it
...
**The load-bearing geometry fact (verified live, backend %31, 2026-06-22):** the live agents
...
- **Long panel (data, 8 subagents):** whole-pane scan (no window) → the cursor (the only agent-row
  `❯`) is the bottom-most `❯`.
