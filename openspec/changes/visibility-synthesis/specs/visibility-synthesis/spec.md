...
(`xo_agent == self`) and its PARENT is in that channel's `members[]` (e.g. `xo_agent=delta-xo
members=[beta-xo]`; `xo_agent=beta-xo members=[alpha-xo]`). Therefore "read the tier BELOW
