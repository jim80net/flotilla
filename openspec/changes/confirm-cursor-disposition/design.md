...
|---|---|---|---|
| backend | `❯ ` then (after submit) the body remains | BLOCKED (submit never lands; PENDING ×25 polls) | blocked (geometry — by luck) |
| data | `❯ Message @reviewer…` (sub-composer, cursor_y=64, ~13 lines above the docked panel) | BLOCKED (wrong focus) | MISSED (composer=undetermined: outside the bottom-10 window) |
| xo | `❯ Press up to edit queued messages` (behind a session-rating modal) | QUEUED → will deliver (soft-success) | mis-reported as failure |
...
above the docked panel (the sub-composer miss). `cursor_y` indexes the captured visible lines 1:1
...
5. **Window expiry:** still **Pending → `ErrPanelBlocked`** (BLOCKED — the body provably remained,
   the backend case; the authority). Never resolved (only Undetermined) → `ErrUnconfirmed`
...
- A LIVE validation pass (a throwaway that runs the real `ComposerState` + a dry classification
  against the live panes) MUST show data=SubAgent(blocked), backend=Pending-after-a-real-
  submit=blocked (or its held state), xo/healthy=Cleared — BEFORE the PR is called clean.
