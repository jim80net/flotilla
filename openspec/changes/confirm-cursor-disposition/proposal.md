...
Live validation (alpha-xo, operator) proved the model wrong on two counts, both invisible to the
synthetic fixtures:

1. **Cursor/geometry position does NOT prove reachability.** `beta-xo`'s cursor sat on the
...
2. **The composer probe was blind to a sub-composer.** `parseComposerPending` scans only the bottom
   ~10 lines. `desk-l` was in a per-agent message SUB-COMPOSER (`❯ Message @reviewer`) ~13 lines
   above the docked panel — OUTSIDE the window → it read "undetermined", so #153 could not even
   measure desk-l's pending state (the false-negative).

A third live state surfaced on `alpha-xo`: `❯ Press up to edit queued messages` — the message was
...
- Manual recovery of the currently-stuck `beta-xo`/`desk-l` (a human keystroke; held).
...
- **Risk:** MEDIUM — reworks the confirm classification. Guarded by: the authority is the
  ...
  (beta-xo pending, desk-l sub-composer, alpha-xo queued) + a LIVE validation pass.
