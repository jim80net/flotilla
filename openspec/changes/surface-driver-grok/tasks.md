...
- [x] 1.1 `internal/surface/grok.go`: `parseGrokState(captured string) State` — last-non-empty bottom-chrome scan (like opencode.go). Claude-style precedence: AwaitingApproval (`Payment required` / `Paste your xAI API key`) → Errored (`An unexpected error occurred.` / `Authentication failed. Your API key may be invalid or expired.` / `Rate limit exceeded. Please wait a moment and try again.`) → Working (`Planning next moves` / `enter queue` / `esc interrupt`) → **Idle (DEFAULT)**. Do NOT rely on the animated spinner `⬒⬔⬓⬕`. Do NOT key on the Plan-mode generic `Confirm`.
- [x] 1.2 Tests (table-driven, EVERY branch): each approval marker → AwaitingApproval; each error literal → Errored; each working marker → Working; idle composer → Idle; **auto-execute case** (a tool running, no approval marker) → Working/Idle NOT AwaitingApproval (proves the reduced set); model output quoting a marker high up → not misled (bottom-chrome scoping).
