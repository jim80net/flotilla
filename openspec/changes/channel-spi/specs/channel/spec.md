...
### Requirement: The coordination bus routes through a per-medium Channel SPI

The system SHALL abstract the coordination bus — the medium the operator and
agents talk over — behind a `Channel` interface that encapsulates the
...
Channels SHALL be looked up from a name-keyed registry (mirroring the surface
`Driver` registry), and an empty channel name SHALL resolve to a default channel
(`discord`) so a configuration that names no medium behaves exactly as before
this change. The relay's pure decision logic (accept/route) SHALL be unchanged
and SHALL remain independent of any concrete medium.

#### Scenario: Default channel is discord

- **WHEN** no coordination channel is named
- **THEN** the bus resolves to the `discord` channel and behaves exactly as before this change

#### Scenario: A channel is selected from the registry by name

- **WHEN** the bus obtains its channel
- **THEN** it looks the channel up from the registry by name (the same name-keyed registry shape as the surface `Driver` registry — `Register` / `Get(name)` with a default), never by constructing a concrete medium type at the call site

#### Scenario: The relay decision logic is medium-agnostic

- **WHEN** an inbound operator message arrives on any channel
...
### Requirement: The Channel interface covers inbound, outbound, and addressing

The `Channel` interface SHALL provide: `Subscribe(ctx, destinations, handler)` —
...
typed value owned by the channel, never a stringly-typed leak of medium internals
...
- **THEN** the channel delivers it to the handler as the narrow projection (origin, id, sender, content), and the relay routes it without referencing any medium-specific type
...
#### Scenario: Addressing resolves through the channel
...
### Requirement: Catch-up is an optional Channel capability

The at-least-once ingestion backstop SHALL be an OPTIONAL `CatchUp` capability a
channel MAY implement (probed by type-assertion, mirroring the surface
`ResultReader` optional capability), NOT a required method of every channel. The
...
channel whose delivery cannot gap (e.g. a loopback in-process medium) SHALL NOT
be required to implement `CatchUp`; callers SHALL type-assert it and skip the
backstop cleanly when it is absent.

#### Scenario: A gap-prone channel supplies the backstop

- **WHEN** the bus reconciles a channel that implements `CatchUp`
...
#### Scenario: A gap-free channel omits the backstop without breaking

- **WHEN** the bus reconciles a channel that does NOT implement `CatchUp` (its delivery cannot gap)
...
resolution) into a single registered `discord` channel obtained through the
...
self-mirror feedback guard (the channel's OWN outbound posts are never re-entered
into the relay) SHALL be preserved as a property enforced inside the channel
...
- **WHEN** an operator message is relayed, a reply is routed, a desk turn is mirrored, or a gateway-gap message is recovered through the extracted `discord` channel
...
- **WHEN** the channel's own outbound (audit-mirror) post appears on a subscribed destination
- **THEN** it is dropped before routing (it never re-enters the relay), exactly as the webhook-author feedback guard does today — the guard is enforced inside the channel adapter and pinned by the existing self-mirror-drop test

### Requirement: A second channel binds loopback-only by construction

The system SHALL provide a second registered channel, `web` (#106), that drives a
loopback web medium through the `Channel` interface — subscribing to operator
...
The `web` channel SHALL bind to a LOOPBACK address only and SHALL REFUSE a
...
never the network. There SHALL be no flag that opens the `web` channel to a
non-loopback interface; widening it is a deliberate, separately-reviewed change.
Adding the `web` channel SHALL NOT change the `discord` channel's behavior.

#### Scenario: A web operator message routes through the shared relay logic

- **WHEN** an operator message arrives on the `web` channel
...
- **WHEN** the `web` channel is constructed with a non-loopback bind address
...
#### Scenario: The web channel needs no catch-up backstop

- **WHEN** the `web` channel is reconciled
...
- **WHEN** the `web` channel is registered alongside `discord`
- **THEN** the `discord` channel's subscribe/post/resolve/catch-up behavior is byte-identical to before, and the default channel remains `discord`
