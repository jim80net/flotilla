...
The `adjutant_for` agent field (legacy alias `assistant_for`) SHALL bind an adjutant
seat to a leader by name. It SHALL NOT alter federation channel membership or synthesis
routing. An adjutant MAY be fleet-internal (no owned channel) while its leader owns a
home channel.
