...
until a human clicks it. The primary XO verified live (2/2: data + backend, 2026-06-22) that
...
catastrophe we are preventing). The primary XO's ×2 worked only because both presses hit STACKED overlays
...
overlay-exit, but the operator + the primary XO + a peer XO all tested Esc and it does NOT recover the
...
- **Named-session resume** (#156 deeper fallback for a genuine freeze that Ctrl-C cannot clear):
  the primary XO has added `--name <role>` to the desk launch recipes so a desk relaunches/resumes by
