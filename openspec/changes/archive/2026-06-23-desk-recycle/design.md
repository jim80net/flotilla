...
beta-xo claude→grok using the same primitive — so #157 is built cross-harness-**ready** (an
...
This design was parlayed with alpha-xo (the remote XO) over a flotilla message on 2026-06-23 (the
...
The first draft templated the handoff/takeover injection from MEMORY of the desk-l `/handoff` and
`/takeover` skills, not from reading them — a direct violation of "read the source, every time."
Reading the skill bodies (`~/.claude/plugins/cache/operator-plugins/desk-l-claude/1.8.0/skills/
...
This parlay surfaced the rule the hard way: alpha-xo (a REMOTE XO over the relay) could not answer an
...
- **Capability-parity is a #158 gate, surfaced not silently degraded:** before the beta-xo
  cutover, #158 confirms Grok's harness supports subagents / parallel-review / git-PR / MCP (beta-xo
  runs multi-agent reviews and owns delta-xo's approval-sensitive order path). If it genuinely
