...
## Decisions carried in (parlayed with the primary XO via flotilla message, 2026-06-23)
...
- **Fork 4 — scope: #157 = the mechanism + a claude→claude same-harness end-to-end proof; #158 = the
  ...
  PR blast-radius bounded (does NOT couple the mechanism to the federated XO's approval-sensitive parity check).
...
from MEMORY of the `/handoff` and `/takeover` skills. Reading the skill bodies shows BOTH are
...
- **#158 — the claude→grok cutover + the capability-parity check** (does Grok's harness support
  subagents / parallel-review / git-PR / MCP, which the federated XO relies on, owning a high-consequence system's
