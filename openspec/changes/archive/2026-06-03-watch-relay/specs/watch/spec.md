...
- **WHEN** the audit mirror posts `→ backend: …` to the channel (a webhook message)
...
- **WHEN** the operator posts `@backend do X` followed by additional lines
- **THEN** the entire multi-line body is delivered to backend as one prompt
