...
(non-empty `WebhookID`, body `→ desk-a: …`) and asserts it is dropped — a
...
`watch` unit passes `--roster`/`--secrets` explicitly (matching the fleet's other
services' env-file convention); no second default-path convention.
...
boundary** (required in the runbook; same posture as a downstream high-consequence agent); (b) a
