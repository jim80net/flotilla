...
**Issue:** #158 — beta-xo migrate Claude Code → Grok surface (operator-directed).
...
recycled context-preserved exactly as a Claude desk is, and so **beta-xo** can migrate Claude
Code → Grok re-activating from a Claude-Code-authored handoff, full XO function intact, on the Grok
subscription (operator-directed spend; desk-e already runs on Grok).
...
beta-xo-on-Grok config is the *circumstantial* instance.
...
### 2a. Live finding (empirical, this session, read-only capture of desk-e pane `%21`)

desk-e was observed **blocked on a tool-approval modal** —
...
- **(C) Orchestrate with existing primitives + the portable-markdown handoff** — RATIFIED (fork-3):
  1. Drive beta-xo (still Claude) to write the handoff to its designated path and commit it —
  ...
  3. `flotilla resume beta-xo` — relaunches via the new (grok) recipe; resume does NOT restore
...
- **HandoffTurn** — the same non-interactive, self-committing instruction as claude's, retargeted:
  ...
  available. Wording must NOT reference claude/desk-l skills; it references the handoff document
...
**Empirical prerequisite:** desk-e cannot be commandeered (active desk; currently mid-decision
...
(Authored here for completeness; EXECUTED by alpha-xo/operator, not auto-fired — beta-xo owns
delta-xo's approval-sensitive order path, so the cutover TIMING is operator-owned.)

1. **Capability-parity check FIRST** (per #158 acceptance): confirm Grok's harness supports
   beta-xo's XO role — multi-agent reviews (silent-failure/OCR subagents), git/PR ops, MCP/tool
   access. If a genuine gap exists, surface it as a finding (do NOT silently degrade the desk).
2. beta-xo (Claude) writes + commits its handoff (claude `HandoffTurn`).
3. Flip roster surface + launch recipe to grok (host-local config).
4. `flotilla resume beta-xo` (relaunch on grok, fresh).
5. Send grok the `TakeoverTurn` at the committed handoff path. **The path is the FROM-harness
   ...
   status record `~/.flotilla/beta-xo/last-recycle.json` `handoff_path` field
...
- The `ComposerStateProbe` capability requirement is NOT yet in the active surface spec — it is parked
  ...
  surface delta carries the requirement. Flag to alpha-xo; do NOT silently duplicate it.
...
**In #158 (operational):** the beta-xo migration runbook + live exercise.
...
## 8. Open items for alpha-xo

1. **fork-3 / option (C)** (orchestrated migration, no new verb) — CONFIRMED by alpha-xo (2026-06-23).
2. **Throwaway-grok characterization session** — APPROVED by alpha-xo (affirmed Grok-subscription
   envelope; the correct cold-test-the-live-artifact step). Proceeding.
3. **beta-xo cutover timing** — CONFIRMED operator-timed by alpha-xo (approval-sensitive order path);
   #158 code + runbook land first; alpha-xo gates/merges the PR like the others.
4. **SSH-agent push blocker** — RESOLVED by alpha-xo: use the gh-token HTTPS bypass (per
...
identity mapping; relevant to where the beta-xo migration writes its persistent XO doctrine on
