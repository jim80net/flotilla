...
- [x] 3.1 TEST FIRST (`recycle_test.go`): grok `HandoffPath(cwd, token)` ==
  ...
  (`git add -f`), the non-interactive/no-confirm instruction, and NO claude harness-specific skill reference;
...
- [x] 5.3 PR via the gh-token HTTPS bypass to the primary XO's gate (reference #158 + this change). Record
