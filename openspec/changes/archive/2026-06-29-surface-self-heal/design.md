...
- **Recovery:** Ctrl-C clears the agents-panel/sub-composer focus back to the composer. The primary XO
  verified live 2/2 (data sub-composer + backend); corroborated by the post-recovery scan (all
  desks `cleared`). Esc — the *documented* overlay-exit — does NOT recover the inline panel (tested
  by the operator, the primary XO, and a peer XO), so Ctrl-C is the empirically-correct mechanism.
...
fixes them structurally. (The primary XO independently affirmed the re-probe-between core; the trio caught
the concurrency surface the primary XO's manual test didn't exercise.)
...
Esc is the DOCUMENTED overlay-exit, but the operator, the primary XO, and a peer XO all tested it and it
...
surfaced to the primary XO. Not in this scope.)
