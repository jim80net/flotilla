...
which side-conversation, what context a newly-dispatched desk is missing. A private
deployment does this by hand in `state/context-ledger.md`; #108 automates it.
...
- **No deployment desk names in the product.** `cos_agent` is a role knob;
  `xo` / `state/context-ledger.md` are a private deployment's operational instance, the
