...
- **WHEN** notify runs with `--from alpha-xo` and the secrets file maps `FLOTILLA_WEBHOOK_ALPHA_XO` to a webhook URL
- **THEN** the request is sent to exactly that URL, with `username = "alpha-xo"`, `Content-Type: application/json`, and flotilla's explicit User-Agent

#### Scenario: Missing webhook for the sender errors by name
- **WHEN** notify runs with `--from alpha-xo` but the secrets file has no `FLOTILLA_WEBHOOK_ALPHA_XO` key
