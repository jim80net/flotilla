...
func runRecycle(ops recycleOps, p recyclePlan) (string, error) {
	...
	case deliver.ResolveNone:
		return "", fmt.Errorf("no pane for %q; nothing to recycle", p.agent)
	...
	if samePaneAsSelf(tid, p.ownPane) {
		return "", fmt.Errorf("refusing to recycle %q: %s is THIS command's own pane — closing it would kill the recycle before the relaunch; run recycle from a different pane or the watch host", p.agent, tid)
	}
	...
	if inMode, err := ops.inMode(target); err != nil {
		...
		return "", fmt.Errorf("refusing to recycle %q: pane %s is in tmux copy/view mode (composer state unreadable) — exit copy-mode, then retry", p.agent, target)
	}
	...
	if err != nil {
		return "", fmt.Errorf("acquire pane transaction lock for %q: %w (another recycle/resume holds it, or the heartbeat is mid-delivery) — ABORT, desk untouched", p.agent, err)
	}
	defer release()
	...
	if err := ops.respawn(target, p.cwd, p.launch); err != nil {
		return "", fmt.Errorf("phase 3: relaunching %q failed: %w (the desk is closed; recover with: flotilla resume %s)", p.agent, err, p.agent)
	}
	...
	if !pollIdleCleared(ops, target, p.timeouts.boot) {
		return "", fmt.Errorf("phase 4: the relaunched %q did not reach idle at a cleared composer within %s — the desk is LIVE but un-taken-over; hand it the chapter with: flotilla send %s 'read %s and take over'", p.agent, p.timeouts.boot, p.agent, p.designatedPath)
	}
	gen, err := ops.readGen(target)
	if err != nil {
		return "", fmt.Errorf("phase 4: reading the recycle generation for %q failed: %w", p.agent, err)
	}
	if gen != p.token {
		return "", fmt.Errorf("phase 4: another recycle superseded %q (generation %q != %q) — abort this takeover", p.agent, gen, p.token)
	}
	if err := ops.deliver(target, p.takeoverText); err != nil {
		return "", fmt.Errorf("phase 4: delivering the takeover turn to %q failed: %w (the desk is LIVE but un-taken-over; hand it the chapter with: flotilla send %s 'read %s and take over')", p.agent, err, p.agent, p.designatedPath)
	}
	...
	return fmt.Sprintf("recycled %s → pane %s (handoff %s; closed gracefully, relaunched fresh, took over)\n", p.agent, target, p.designatedPath), nil
}
...
func cmdRecycle(args []string) error {
	agentName, rosterPath, launchPath, dryRun, err := parseRecycleArgs(args)
	if err != nil {
		return err
	}
	...
	msg, runErr := runRecycle(ops, plan)
	writeLastRecycle(agentName, plan, msg, runErr)
	if runErr != nil {
		return runErr
	}
	fmt.Print(msg)
...
func writeLastRecycle(agent string, p recyclePlan, msg string, runErr error) {
...
func parseRecycleArgs(args []string) (agent, rosterPath, launchPath string, dryRun bool, err error) {
	...
	if agent == "" || len(rest) != 0 {
		return "", "", "", false, fmt.Errorf("usage: flotilla recycle <agent> [--launch <path>] [--dry-run]")
	}
	return agent, *rp, *lp, *dr, nil
}
