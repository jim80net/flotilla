...
func TestMirrorRelay_InboundLedgeredWithOriginChannel(t *testing.T) {
	ledger := filepath.Join(t.TempDir(), "context-ledger.md")
	...
	mirrorRelayToLedger(ledger, watch.Job{Agent: "alpha-xo", Message: "ship the cache PR", Kind: "relay", OriginChannel: "C_ALPHA"})
...
func TestMirrorRelay_InertWhenCosLedgerEmpty(t *testing.T) {
	// cos_agent unset ⇒ cosLedger == "" ⇒ no write (and no panic on the empty path).
	mirrorRelayToLedger("", watch.Job{Agent: "alpha-xo", Message: "x", OriginChannel: "C_ALPHA"})
}
