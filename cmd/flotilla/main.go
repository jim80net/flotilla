...
usage:
  ...
  flotilla accounts refresh [--probe-only [--json] | --yes] <subscription-id> inspect or renew isolated Claude OAuth credentials
...
flags for 'dash':
  ...
  --frontier-source <coordinator=path>
                            publish an owner-filtered return-to source (repeatable; backlog-file maps only the primary XO)
