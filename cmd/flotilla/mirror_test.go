...
func TestDeskMirrorSkipsWhenNoWebhook(t *testing.T) {
	...
	if len(lines) != 1 || !strings.Contains(lines[0], "SKIP desk-a: no webhook") {
...
func TestDeskMirrorSkipsWhenNotSubstantive(t *testing.T) {
	...
	if len(lines) != 1 || !strings.Contains(lines[0], "SKIP desk-a: no substantive") {
...
func TestDeskMirrorSkipsOnReadError(t *testing.T) {
	...
	m.run("desk-a")
	if len(lines) != 1 || !strings.Contains(lines[0], "SKIP desk-a: read turn-final: tmux boom") {
...
func TestDeskMirrorPostsSingleChunk(t *testing.T) {
	...
	m.run("desk-a")
	if gotURL != "https://wh" || gotUser != "desk-a" {
	...
	if len(lines) != 1 || !strings.Contains(lines[0], "POST desk-a 1 chunks") {
...
func TestDeskMirrorChunksOversizeAndPrefixes(t *testing.T) {
	...
	if len(lines) != 1 || !strings.Contains(lines[0], "POST desk-a 3 chunks") {
...
func TestDeskMirrorStopsAndLogsOnPostFailure(t *testing.T) {
	...
	if len(lines) != 1 || !strings.Contains(lines[0], "MIRROR-FAIL desk-a: chunk 2/3") {
