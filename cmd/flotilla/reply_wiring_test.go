...
func TestIsHotlineToChannelXO_LegacyPrimary(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "roster.json")
	js := `{"agents":[{"name":"xo"},{"name":"backend"}],"xo_agent":"xo",
	...
	if !isHotlineToChannelXO(cfg, watch.Job{Kind: "relay", Agent: "xo", OriginChannel: "C_MAIN"}) {
		t.Error("legacy single-channel primary XO should ARM (#177 unified)")
	}
	// a heartbeat tick to the primary XO must NOT arm.
	if isHotlineToChannelXO(cfg, watch.Job{Kind: "heartbeat", Agent: "xo", OriginChannel: "C_MAIN"}) {
