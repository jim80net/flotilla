...
func TestParseRegisterArgs(t *testing.T) {
	cases := []struct {
		...
		{"agent then flag (the migration form)", []string{"backend", "--pane", "0:0.2"}, "", "backend", "0:0.2", false},
		{"flag then agent", []string{"--pane", "0:0.2", "backend"}, "", "backend", "0:0.2", false},
		{"agent only, pane from default ($TMUX_PANE)", []string{"backend"}, "%7", "backend", "%7", false},
		{"agent with =flag form", []string{"backend", "--pane=%9"}, "", "backend", "%9", false},
