...
func TestReaderModelInternal_AbsentIsBackCompat(t *testing.T) {
	body, note, suppress := readerModelInternal("just an ordinary turn-final")
	if suppress {
		t.Fatal("nothing suppresses on the internal mirror in P0")
	}
	if body != "just an ordinary turn-final" {
		t.Errorf("absent envelope must pass the raw body through; got %q", body)
	}
	if note != "" {
		t.Errorf("an ordinary turn-final carries no flag; got note %q", note)
	}
}
