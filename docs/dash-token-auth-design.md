...
- **The open-set strings MUST equal the mux's registered patterns (folded: step-5 P1).** `mux.Handler(r)` returns the *literal registered* pattern, so the static route must be registered **`GET /static/`** (it is bare `/static/` today, `server.go:192`) for the `"GET /static/"` open entry to match — else static assets get gated and the login UI 401s. Re-registering it `GET /static/` also method-gates static to GET. `POST /api/auth/login|logout` are registered with method prefixes (correct). Only the OPEN-set patterns must match exactly; **gated** routes are simply absent from `openPatterns`, so their exact pattern string is irrelevant to the gate (they're denied either way). (The read routes are registered bare today, e.g. `/api/status` — fine, they're gated by absence.)
