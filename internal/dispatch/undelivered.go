...
func ReconcileInboundAcksWithTerminal(rosterDir string, readTurnFinal TurnFinalReader, isMerged RecipientMergedChecker, isCommitOnMain RecipientCommitChecker) int {
	...
	for _, path := range inbound.ListInboundPaths(rosterDir) {
		...
		if isMerged != nil || isCommitOnMain != nil {
			for _, e := range st.Load() {
				...
				if evidence, terminal := ShouldSuppressTerminal(e.Message, prChecker, commitChecker); terminal {
					if _, cerr := reg.Consume(ConsumeFromInbound(e.Nonce, e.Message, ReasonMerged, e.Sender, e.Recipient, e.ID)); cerr != nil {
		...
		for _, e := range st.ClearAcknowledged(text) {
			n++
			if _, cerr := reg.Consume(ConsumeFromInbound(e.Nonce, e.Message, ReasonTurnFinalAck, e.Sender, e.Recipient, e.ID)); cerr != nil {
