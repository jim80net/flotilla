...
func TestParsePane(t *testing.T) {
	...
	got, err := parsePane(out, "frontend")
	if err != nil {
...
func TestParsePaneAmbiguousIsError(t *testing.T) {
	...
	out := "0:0.0\tfrontend\n1:0.0\tfrontend\n"
	if _, err := parsePane(out, "frontend"); err == nil {
...
func TestParsePaneExactMatchNotSubstring(t *testing.T) {
	...
	out := "0:0.0\tfrontend\n"
	if _, err := parsePane(out, "front"); err == nil {
...
func TestParsePaneMatchesGlyphPrefixedTitle(t *testing.T) {
	...
	// The glyph prefix must not let "front" match "✳ frontend".
	if _, err := parsePane(out, "front"); err == nil {
		t.Error("parsePane(front) matched a glyph-prefixed frontend, want no match")
	}
}

func TestParsePaneMarkerResolvesDriftedTitle(t *testing.T) {
	// The bug: a Claude desk launched as "data" retitled itself to a
	// task summary. With the stable @flotilla_agent marker it still resolves —
	// the title is now irrelevant.
	out := "0:0.0\t⠂ xo\t\n" +
		"0:0.2\t✳ Design the new feature\tdata\n"
	got, err := parsePane(out, "data")
	if err != nil {
...
func TestParsePaneMarkerWinsOverTitleOfAnotherPane(t *testing.T) {
	...
	out := "0:0.1\t✳ some drifted summary\tfrontend\n" + // marker pane (drifted title)
...
func TestParsePaneMarkerDoesNotMatchEmptyWantOrEmptyMarker(t *testing.T) {
	...
	out := "0:0.0\tsomething\t\n0:0.1\tother\t\n"
	if _, err := parsePane(out, "data"); err == nil {
...
func TestParseFieldsRobustToTabInTitle(t *testing.T) {
	...
	cases := []struct {
		...
		{"0:0.0\tplain title\tfrontend", "0:0.0", "plain title", "frontend"},
		{"0:0.0\thas\ta\ttab\tfrontend", "0:0.0", "has\ta\ttab", "frontend"}, // tabs in title
...
func TestTitleMatches(t *testing.T) {
	cases := []struct {
		...
		{"frontend", "frontend", true},   // bare name
		{"✳ frontend", "frontend", true}, // single-glyph prefix (Claude live title)
		{"⠂ xo", "xo", true},             // different spinner glyph
		{"valbot", "valbot", true},
		{"✳ frontend", "front", false},     // substring must not match
		{"my frontend", "frontend", false}, // multi-word prefix is not a glyph
		{"build frontend", "frontend", false},
		{"foo bar frontend", "frontend", false},
...
func TestSendMenuNavigationArgsAreSingleGuardedKeys(t *testing.T) {
	for _, key := range []string{"Up", "Down"} {
		got := sendMenuNavigationArgs("-dash:1.2", key)
		want := []string{"send-keys", "-t", "-dash:1.2", "--", key}
		if !reflect.DeepEqual(got, want) {
			t.Fatalf("sendMenuNavigationArgs(%q) = %v, want %v", key, got, want)
		}
	}
}
