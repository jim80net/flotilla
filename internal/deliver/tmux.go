...
// "✳ frontend"), so exact equality fails on a live session. We accept the bare
// name, or a SINGLE-rune glyph prefix followed by the name. Constraining the
// prefix to one rune rejects both "front" (substring) and "build frontend"
// (an unrelated multi-word pane) as matches for "frontend". This is the FALLBACK
