...
//
// Routing policy (#533): when adjutant_for is set, all coordinator notifications
// route to the adjutant regardless of source/kind/priority/bypass label. The adjutant
// may interrupt the leader only via KindAdjutantSeam drain. Urgent/operator-direct
// leader bypass routing is deferred until a concrete consumer exists. Posture alone
