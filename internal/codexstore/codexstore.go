...
func rolloutPathsForProcess(codexHome, cwd string, panePID int, procRoot string) ([]string, error) {
	...
	if strings.TrimSpace(cwd) == "" {
		return nil, fmt.Errorf("codex store: pane cwd is empty; refusing unverifiable result")
	}
	...
	for path := range paths {
		...
		// The process-bound open descriptor selects candidate rollouts. Some Codex
		// sessions emit an empty cwd (or no session_meta), so only populated metadata
		// can act as the fail-closed collision guard introduced by #826.
