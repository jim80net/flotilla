...
func (claudeCode) TakeoverTurn(designatedPath string) string {
	return "You are a freshly-recycled flotilla desk with a clean context window, and you are " +
		...
		"2. Then, as your FIRST action, REMOVE the transferred handoff from version control so it " +
		"cannot leak into a later public PR (it is gitignored and carries deployment specifics; it was " +
		"committed only to durably transfer it across the recycle, and you have read it now): " +
