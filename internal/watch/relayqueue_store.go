...
type pendingRelay struct {
	...
	LastBufferFailureAlert time.Time `json:"last_buffer_failure_alert_at,omitempty"`
}
...
func (s relayQueueStore) load() []Job {
	...
	for _, p := range f.Pending {
		...
		out = append(out, Job{
			...
			lastBufferFailureAlert: p.LastBufferFailureAlert,
...
func (s relayQueueStore) upsert(j Job) {
	...
	entry := pendingRelay{
		...
		LastBufferFailureAlert: j.lastBufferFailureAlert,
	}
	replaced := false
...
func queueEntryMateriallyChanged(prev, next pendingRelay) bool {
	...
	if prev.LastBufferFailureAlert.IsZero() != next.LastBufferFailureAlert.IsZero() {
		return true
	}
	if !prev.LastBufferFailureAlert.Equal(next.LastBufferFailureAlert) {
		return true
	}
	return false
}
