...
func TestHeartbeatDisabled(t *testing.T) {
	c := &collector{}
	h := NewHeartbeat(0, "xo", "", c.enqueue, nil)
	h.Start()
...
func TestHeartbeatFiresWhenIdle(t *testing.T) {
	c := &collector{}
	h := NewHeartbeat(20*time.Millisecond, "xo", "", c.enqueue, func(string) bool { return false })
	h.Start()
	...
	if c.jobs[0].Agent != "xo" || c.jobs[0].Message != DefaultHeartbeatPrompt {
...
func TestHeartbeatSkippedWhenBusy(t *testing.T) {
	c := &collector{}
	var busy int32 = 1
	h := NewHeartbeat(20*time.Millisecond, "xo", "", c.enqueue,
...
func TestHeartbeatResetSuppressesTick(t *testing.T) {
	c := &collector{}
	h := NewHeartbeat(80*time.Millisecond, "xo", "", c.enqueue, func(string) bool { return false })
	h.Start()
...
func TestHeartbeatActivityStableFires(t *testing.T) {
	c := &collector{}
	h := NewHeartbeat(40*time.Millisecond, "xo", "", c.enqueue, func(string) bool { return false })
	h.SetPollInterval(10 * time.Millisecond)
