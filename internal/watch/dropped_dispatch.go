...
func InboundTrackHook(rosterDir string, isCoordinator inbound.CoordinatorPredicate) func(Job) {
	...
	return func(j Job) {
		...
		if decision == inbound.TrackSkipped {
			if _, err := dispatch.ConsumeCoordinatorRecipientID(rosterDir, j.Sender, j.Agent, j.MessageID, j.Message); err != nil {
	...
	return func(agent string) {
		...
		for _, e := range st.Load() {
			if pr, merged := dispatch.ShouldSuppressMerged(e.Message, isMerged); merged {
				if _, cerr := reg.Consume(dispatch.ConsumeFromInboundEntry(e, dispatch.ReasonMerged)); cerr != nil {
		...
		for _, e := range st.Load() {
			...
			if _, cerr := reg.Consume(dispatch.ConsumeFromInboundEntry(e, dispatch.ReasonQueuedAck)); cerr != nil {
		...
		for _, a := range st.OnFinish(text) {
			if a.Reinject {
				...
				if pr, merged := dispatch.ShouldSuppressMerged(a.Entry.Message, isMerged); merged {
					if _, cerr := reg.Consume(dispatch.ConsumeFromInboundEntry(a.Entry, dispatch.ReasonMerged)); cerr != nil {
		...
		for _, e := range pendingBefore {
			...
			if _, cerr := reg.Consume(dispatch.ConsumeFromInboundEntry(e, dispatch.ReasonTurnFinalAck)); cerr != nil {
