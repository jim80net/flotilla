...
func TestRelayQueueStoreRoundTrip(t *testing.T) {
	...
	j := Job{
		...
		lastBufferFailureAlert: time.Date(2026, 7, 3, 5, 1, 0, 0, time.UTC),
	}
	q.upsert(j)
	got := q.load()
	if len(got) != 1 {
	...
	if got[0].MessageID != j.MessageID || got[0].deferrals != 3 || got[0].Agent != "cos-adj" || got[0].IntendedRecipient != "cos" || !got[0].lastBufferFailureAlert.Equal(j.lastBufferFailureAlert) {
