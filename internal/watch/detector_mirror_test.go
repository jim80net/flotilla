...
func TestDetectorMirrorsNonXODeskOnFinish(t *testing.T) {
	var mu sync.Mutex
	var mirrored []string
	cfg := mirrorConfig("alpha-xo", []string{"alpha-xo", "desk-a"}, func(a string) {
		mu.Lock()
		defer mu.Unlock()
	...
	mu.Lock()
	defer mu.Unlock()
	if len(mirrored) != 1 || mirrored[0] != "desk-a" {
		t.Errorf("MirrorOnFinish calls = %v, want exactly [desk-a]", mirrored)
	}
}
...
func TestDetectorMirrorDoesNotBlockOperatorWake(t *testing.T) {
	...
	cfg := mirrorConfig("alpha-xo", []string{"alpha-xo", "desk-a"}, func(string) {
