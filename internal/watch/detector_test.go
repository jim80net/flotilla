...
func TestDetectorDeskFinishedWakesTargeted(t *testing.T) {
	...
	f.set("alpha-xo", surface.StateIdle)
	f.set("desk-a", surface.StateIdle) // desk finished a turn
	f.signal = "h0"
	...
	if len(f.lastWake().reasons) != 1 || f.lastWake().reasons[0][:6] != "desk-a" {
...
func TestDetectorIdleFleetIsSilent(t *testing.T) {
	...
	cfg := f.config("alpha-xo", []string{"alpha-xo", "desk-a"}, 3, "none")
	d := newDet(t, f, cfg)
	seed(d, map[string]surface.State{"alpha-xo": surface.StateIdle, "desk-a": surface.StateIdle}, "h0")
	f.set("alpha-xo", surface.StateIdle)
...
func TestDetectorShellDebounced(t *testing.T) {
	...
	seed(d, map[string]surface.State{"alpha-xo": surface.StateIdle, "desk-a": surface.StateIdle}, "h0")
	f.set("alpha-xo", surface.StateIdle)
	f.set("desk-a", surface.StateShell) // first shell read — a blip
	f.signal = "h0"
	...
	f.reset()
	d.Tick()
	if f.wakeCount() != 1 || f.lastWake().reasons[0][:6] != "desk-a" {
...
func TestDetectorXOSelfContinuationOnceNotDeskFinished(t *testing.T) {
	...
	cfg := f.config("alpha-xo", []string{"alpha-xo", "desk-a"}, 3, "none")
	d := newDet(t, f, cfg)
	seed(d, map[string]surface.State{"alpha-xo": surface.StateWorking, "desk-a": surface.StateIdle}, "h0")
	f.set("alpha-xo", surface.StateIdle) // XO finished a turn
	f.set("desk-a", surface.StateIdle)
	f.signal = "h0"
...
func TestDetectorLivenessWedgeAndRecovery(t *testing.T) {
	f := newFixture()
	cfg := f.config("alpha-xo", []string{"alpha-xo"}, 3, "interval") // alert at K=3 intervals
	d := newDet(t, f, cfg)
	seed(d, map[string]surface.State{"alpha-xo": surface.StateIdle}, "h0")
	f.set("alpha-xo", surface.StateIdle)
	f.signal = "h0"
...
func TestDetectorLivenessCrashImmediate(t *testing.T) {
	f := newFixture()
	cfg := f.config("alpha-xo", []string{"alpha-xo"}, 99, "none") // huge K — crash must still be immediate
	d := newDet(t, f, cfg)
	seed(d, map[string]surface.State{"alpha-xo": surface.StateIdle}, "h0")
	f.set("alpha-xo", surface.StateShell)
	f.signal = "h0"
...
func TestDetectorWedgeSuppressedWhileShellPending(t *testing.T) {
	...
	f := newFixture()
	cfg := f.config("alpha-xo", []string{"alpha-xo"}, 3, "interval") // alert at 3 intervals
	d := newDet(t, f, cfg)
	seed(d, map[string]surface.State{"alpha-xo": surface.StateIdle}, "h0")
	f.set("alpha-xo", surface.StateShell)
	f.signal = "h0"
...
func TestDetectorMaxQuietPing(t *testing.T) {
	f := newFixture()
	cfg := f.config("alpha-xo", []string{"alpha-xo"}, 3, "interval") // pingEvery = K-1 = 2
	d := newDet(t, f, cfg)
	seed(d, map[string]surface.State{"alpha-xo": surface.StateIdle}, "h0")
	f.set("alpha-xo", surface.StateIdle)
	f.signal = "h0"
