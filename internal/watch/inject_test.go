...
func TestInjectorLogsSuccessfulDelivery(t *testing.T) {
	...
	cases := []struct {
		...
		{"relay", Job{Agent: "desk-a", Message: "ship it", Kind: "relay"}, "relay", len("ship it")},
		{"heartbeat", Job{Agent: "alpha-xo", Message: "tick-tick", Kind: "heartbeat"}, "heartbeat", len("tick-tick")},
...
func TestInjectorPanelBlockedRelayRaisesActionableAlert(t *testing.T) {
	...
	in.Start()
	in.Enqueue(Job{Agent: "beta-xo", Message: "please run the edge audit", Kind: "relay"})
	in.Stop()
	...
	a := alerts[0]
	for _, want := range []string{"beta-xo", "agents panel", "keystroke", "please run the edge audit"} {
...
func TestInjectorPanelBlockedTickDoesNotAlarm(t *testing.T) {
	...
	in.Start()
	in.Enqueue(Job{Agent: "desk-l", Message: "tick", Kind: "heartbeat"})
	in.Stop()
