...
func TestIssuesWorkLedger405(t *testing.T) {
	...
	css := doGet(t, srv, "/static/dash.css").Body.String()
	for _, marker := range []string{".issue-ledger-section", ".issue-ledger-head", ".issue-desk-head", ".issue-context", ".issue-state.shipped", ".issue-shipped-more", ".issue-shipped-more:not([open]) > .issue-row", ".issue-scope-note"} {
		if !strings.Contains(css, marker) {
...
func TestIssuesWorkContext716(t *testing.T) {
	...
	for _, marker := range []string{
		...
		`Timeline sources did not respond within`, `D.clearJSON(path)`,
	} {
		if !strings.Contains(js, marker) {
	...
	dashJS := doGet(t, srv, "/static/dash.js").Body.String()
	for _, marker := range []string{"renderMirrorEntries", "routeMessage", "routeOutcomeCopy", "onLiveUpdate", "clearJSON: clearJSON"} {
		if !strings.Contains(dashJS, marker) {
...
func TestMobileWorkContextDensity725(t *testing.T) {
	...
	for _, marker := range []string{".wc-message-clamped:not(.is-expanded)", "-webkit-line-clamp: 8", ".wc-live-contract.is-idle",
		...
		".wc-seat { white-space: nowrap; max-width: none; overflow: visible; }"} {
		if !strings.Contains(css, marker) {
