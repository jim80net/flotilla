...
func TestResearchPageAndDashboardNavMarkers(t *testing.T) {
	...
	if !strings.Contains(page, `aria-label="Detail page navigation"`) || !strings.Contains(page, `class="pd-topback detail-back"`) || !strings.Contains(page, `← Dashboard`) {
		t.Error("R&D must expose the shared deliberate Dashboard detail-page control")
	}
