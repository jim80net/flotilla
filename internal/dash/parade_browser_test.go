...
def assert_controls_contained(page, selector, width):
    result = page.evaluate("""selector => {
      const boxes = [...document.querySelectorAll(selector)]
        .filter(node => !node.hidden && getComputedStyle(node).display !== "none")
        .map(node => {
          const box = node.getBoundingClientRect();
          return {id: node.id || node.className, left: box.left, right: box.right};
        });
      return {
    ...
    assert result["scroll"] == result["client"] == width, result
    assert result["boxes"], result
    for box in result["boxes"]:
        assert box["left"] >= 0 and box["right"] <= width, result
...
with sync_playwright() as p:
    ...
    try:
        ...
        assert_controls_contained(page, "#pd-deck .pd-deck-bar > *", 390)
        ...
        assert_controls_contained(page, "#pd-list-view .parade-topnav > *", 390)
        if evidence_dir:
